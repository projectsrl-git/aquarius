...
DO CASE
   CASE V_SCELTA=1          				&& ACCESSO CODICE CLIENTE
        IF PUB_VISRIC 
           ...
           ENDIF
           _ii_cSql = "select * from u_lis_al where " + x_cond + ;
                                                    " order by lis_codsoc,lis_codcli,lis_codart,lis_alteda" 
        ELSE
           IF SUBSTR(V_SEEK,1,1)  <  "A"
	          V_SEEK = TRC(PUB_LCL,ALLTRIM(V_SEEK))
	       ENDIF
   ...
   CASE V_SCELTA=2                          && ACCESSO PER RAGIONE SOCIALE
        IF PUB_VISRIC 
           IF EMPTY(V_SEEK) .OR. SUBSTR(V_SEEK,1,5) = SPACE(05)
           ELSE
              x_codi = "lis_ragsoc like '" + ALLTRIM(V_SEEK) + "%'"
              x_cond = x_cond + " and " + x_codi
           ENDIF
           _ii_cSql = "select * from u_lis_al where " + x_cond + ;
                                                    " order by lis_codsoc,lis_ragsoc,lis_codart,lis_alteda"
        ELSE
           x_codi = "lis_ragsoc like '" + ALLTRIM(V_SEEK) + "%'"
           x_cond = x_cond + " and " + x_codi
        ENDIF
   ...
   CASE V_SCELTA=3                          && ACCESSO PER CODICE ARTICOLO
        IF PUB_VISRIC 
           ...
           ENDIF
           _ii_cSql = "select * from u_lis_al where " + x_cond + ;
                                                    " order by lis_codsoc,lis_codart,lis_alteda,lis_codcli" 
        ELSE
           x_codi = "lis_codart = '" + ALLTRIM(V_SEEK) + "'"
           x_cond = x_cond + " and " + x_codi
        ENDIF
   ...
   CASE V_SCELTA=4          				&& ACCESSO PER DATA LISTINO
        IF PUB_VISRIC 
           ...
           ENDIF
           _ii_cSql   = "select * from u_lis_al where " + x_cond + ;
                                                      " order by lis_codsoc,lis_datlis,lis_codcli,lis_codart,lis_alteda" 
        ELSE
           x_data = "lis_datlis = '" + RIBALTA2(ALLTRIM(V_SEEK)) + "'"
           x_cond = x_cond + " and " + x_data
        ENDIF
