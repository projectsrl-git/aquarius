...
DO CASE
   ...
   CASE V_SCELTA=1          				&& ACCESSO PER NUMERO
        ...
        ELSE
           ...
           x_cond   = x_cond + " and " + x_nord
        ENDIF
        _ii_cSql = "select * from u_FIN_tt where " + x_cond + ;
                                                 " order by FIN_codsoc,FIN_NUMCON" 

   CASE V_SCELTA=2                          && ACCESSO PER DATA
        IF PUB_VISRID 
           IF EMPTY(V_SEEK) .OR. SUBSTR(V_SEEK,1,5) = SPACE(05)
           ELSE
              x_data = "FIN_DATCON = '" + RIBALTA2(ALLTRIM(V_SEEK)) + "'"
        ...
        ENDIF
        _ii_cSql   = "select * from u_FIN_tt where " + x_cond + ;
                                                   " order by FIN_codsoc,FIN_DATCON,FIN_NUMCON" 

   CASE V_SCELTA=3          				&& ACCESSO PER RAGIONE SOCIALE
        IF PUB_VISRID 
           IF EMPTY(V_SEEK) .OR. SUBSTR(V_SEEK,1,5) = SPACE(05)
           ELSE
              x_rags = "FIN_ragsoc like '" + ALLTRIM(V_SEEK) + "%'"
        ...
        ENDIF
        _ii_cSql   = "select * from u_FIN_tt where " + x_cond + ;
                                                   " order by FIN_codsoc,FIN_ragsoc" 
   CASE V_SCELTA=4          				&& ACCESSO PER CODICE PDV
        IF PUB_VISRID 
           IF EMPTY(V_SEEK) .OR. SUBSTR(V_SEEK,1,5) = SPACE(05)
           ELSE
              x_cage = "FIN_codpdv = '" + ALLTRIM(V_SEEK) + "'"
        ...
        ENDIF
        _ii_cSql   = "select * from u_FIN_tt where " + x_cond + ;
                                                   " order by FIN_codsoc,FIN_codpdv,ord_DATCON" 
   ...
   CASE V_SCELTA=5          				&& ACCESSO PER CODICE CLIENTE
        IF PUB_VISRID 
           ...
           ELSE
              ...
              x_clie = "FIN_codcli = '" + ALLTRIM(V_SEEK) + "'"
        ...
        ENDIF
        _ii_cSql   = "select * from u_FIN_tt where " + x_cond + ;
                                                   " order by FIN_codsoc,FIN_codcli,FIN_DATCON" 
