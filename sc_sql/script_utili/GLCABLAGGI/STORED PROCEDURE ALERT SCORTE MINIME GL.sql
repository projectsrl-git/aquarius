...
/****** Object:  StoredProcedure [dbo].[ALERT_SCORTE_MINIME]    Script Date: 06/06/2023 12:13:14 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SET @NR_ARTICOLI_SOTTOSCORTA = (SELECT COUNT(*) FROM (SELECT ART_CODPRI,CAST(ART_SCORMI AS numeric(20,2)) AS ART_SCORMI,CAST(COALESCE((SELECT ROUND(SUM(MAG_GIACEN),2) FROM GLCABLAGGI.dbo.U_MAG_GG WHERE MAG_CODMAG = ART_MAGA AND MAG_ANAART = ART_CODPRI),0) AS numeric(20,2)) AS MAG_GIACEN FROM GLCABLAGGI.dbo.U_ART_PR WHERE ART_SCORMI > 0) AS MAIN WHERE MAG_GIACEN < ART_SCORMI)
	...
	IF (@NR_ARTICOLI_SOTTOSCORTA > 0)
		BEGIN
			...
			SELECT @sub = 'Alert Scorte minime magazzino GL Cablaggi';
			...
			FROM (
				SELECT * FROM (
						SELECT 
							...
							CAST(COALESCE((SELECT ROUND(SUM(MAG_GIACEN),2) FROM GLCABLAGGI.dbo.U_MAG_GG WHERE MAG_CODMAG = ART_MAGA AND MAG_ANAART = ART_CODPRI),0) AS numeric(20,2)) AS MAG_GIACEN
						FROM GLCABLAGGI.dbo.U_ART_PR
						WHERE ART_SCORMI > 0
					) AS MAIN
			...
			EXEC msdb.dbo.sp_send_dbmail
				 ...
				 @recipients = 'p.bollani@glcablaggi.it',
