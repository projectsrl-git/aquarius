...
) AS ELENCO_BARRE ORDER BY MAG_CODMAG,MAG_DATAGG,MAG_CODART



-- QTA TOTALE BARRE PRESENTI IN MAGAZZINO MA CHE IN TEORIA DOVREBBERO ESSERE GIA' STATE SCARICATE 
-- CONSIDERANDO LE TABELLE DI PRODUZIONE

SELECT SUM(QTAMQ) AS QTAMQ_TOT FROM (

	SELECT MAG_CODART,MAG_ALTEZZ,MAG_LUNGHE,MAG_ALTEZZ*MAG_LUNGHE/1000 AS QTAMQ,MAG_GIACEN
	FROM U_MAG_GG 
				   INNER JOIN u_prd_d4
					 ON u_prd_d4.pr4_codint = u_mag_gg.mag_codart
				   INNER JOIN (select distinct pj_prgprd1,pj_prgprd2,pj_prgprd3,pj_prgprd4 from u_prd_d5 where pr5_codint <> '') as d5
					 ON d5.pj_prgprd1 = u_prd_d4.pj_prgprd1
						AND d5.pj_prgprd2 = u_prd_d4.pj_prgprd2
						AND d5.pj_prgprd3 = u_prd_d4.pj_prgprd3
						AND d5.pj_prgprd4 = u_prd_d4.pj_prgprd4
	...
	FROM U_MAG_GG 
				   INNER JOIN u_prd_d4
					 ON u_prd_d4.pr4_codint = u_mag_gg.mag_codart
				   INNER JOIN (select distinct pj_prgprd1,pj_prgprd2,pj_prgprd3,pj_prgprd4 from u_prd_pk where pr5_codint <> '') as pk
					 ON pk.pj_prgprd1 = u_prd_d4.pj_prgprd1
						AND pk.pj_prgprd2 = u_prd_d4.pj_prgprd2
						AND pk.pj_prgprd3 = u_prd_d4.pj_prgprd3
						AND pk.pj_prgprd4 = u_prd_d4.pj_prgprd4
