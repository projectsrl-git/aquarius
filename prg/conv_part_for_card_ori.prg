...
*** conferma utente
		...
		RIT = MESSAGEBOX("Procedo con la conversione "+CHR(13)+"del partitario fornitori CARDIOLINE in formato TXT ?",292,"ATTENZIONE")
		IF RIT = 7
...
****** DEFINIZIONE DEI CURSORI STP
		...
		IF !ExecCommand("select * from PART_FOR where 1=0","cursore_PART_FOR")
		   return
		ENDIF
...
*** CANCELLAZIONE DEI RECORD DEI PARTITARI SQL 
		...
		RIT = MESSAGEBOX("Desideri cancellare "+CHR(13)+"il partitario fornitori CARDIOLINE ?",292,"ATTENZIONE")
		IF RIT = 7
		ELSE 
		    WAIT WINDOWS "ATTENDERE PREGO, CANCELLAZIONE ARCHIVI PARTITARI FORNITORI" NOWAIT
			cSql = "delete from PART_FOR"
...
_VALUTA      	= "EURO"
_CTR1 			= 0
PRIMO_FORNITORE   = .T.	&& PRIMO CLIENTE
...
*!*	#####################################################
	...
	IF !EMPTY(SUBSTR(CAMPO1,01,09))
	   ...
	   =DECODIFICA_FORNITORE(M.PAR_CODCLI)
...
*!*		ENDIF
	...
	M.PAR_NUMFAT = ALLTRIM(SUBSTR(CAMPO1,120,10))	
	M.PAR_NPROT  = ALLTRIM(CONV_CLEAR(M.PAR_NUMFAT))
   	M.PAR_NUMPRO = SYS(2015)

*!*	IF ALLTRIM(M.PAR_NUMFAT) = '30486067' 
*!*	SET STEP ON 
*!*	ENDIF 

	M.PAR_DTREG    	= RIBALTA2(SUBSTR(CAMPO1,11,10))
	M.PAR_NREG     	= M.PAR_NUMFAT

	M.PAR_DTFAT  = RIBALTA2(SUBSTR(CAMPO1,162,10))
	IF EMPTY(ALLTRIM(M.PAR_DTFAT))
		M.PAR_DTFAT  = M.PAR_DTREG
	ENDIF
    M.PAR_DTFAT  = ALLTRIM(CONV_CLEAR(M.PAR_DTFAT))


    IF LEN(ALLTRIM(M.PAR_NUMFAT)) > 6
		
		_NUMFAT___ = M.PAR_NUMFAT
		IF LEN(ALLTRIM(M.PAR_NUMFAT)) = 10
			_NUMFAT___ = SUBSTR(M.PAR_NUMFAT,5,6)
		ENDIF 
		IF LEN(ALLTRIM(M.PAR_NUMFAT)) = 9
			_NUMFAT___ = SUBSTR(M.PAR_NUMFAT,4,6)
		ENDIF 
		IF LEN(ALLTRIM(M.PAR_NUMFAT)) = 8
			_NUMFAT___ = SUBSTR(M.PAR_NUMFAT,3,6)
		ENDIF 
		IF LEN(ALLTRIM(M.PAR_NUMFAT)) = 7
			_NUMFAT___ = SUBSTR(M.PAR_NUMFAT,2,6)
		ENDIF 
		IF LEN(ALLTRIM(M.PAR_NUMFAT)) = 6
			_NUMFAT___ = SUBSTR(M.PAR_NUMFAT,1,6)
		ENDIF 
		M.PAR_NPART = VAL(SUBSTR(M.PAR_DTFAT,1,4) + _NUMFAT___)	
    ELSE
		M.PAR_NPART = VAL(SUBSTR(M.PAR_DTFAT,1,4) + M.PAR_NUMFAT)	
    ENDIF

	M.PAR_DTSCAD  = ""	
	IF !EMPTY(SUBSTR(CAMPO1,219,10))
		M.PAR_DTSCAD = RIBALTA2(SUBSTR(CAMPO1,219,10))
	ENDIF
...
****** AGGIORNA TABELLA PART_FOR
IF !ExecRW('','PART_FOR','I','','cursore_PART_FOR')
   return(.f.)
ENDIF
...
___NAZION    = SPACE(03)
	...
	SELECT cu_for_an	
	IF RECCOUNT() <> 0
      M.PAR_RAGSOC = FOR_RAGSOC
      M.PAR_CODPAG = FOR_CONPAG
      M.PAR_TEL    = FOR_TELEFO
      M.PAR_IVA    = FOR_PARIVA
	  M.PAR_BANCA  = FOR_BAE
	  IF EMPTY(M.PAR_BANCA)
		M.PAR_BANCA  = FOR_BA2
	  ENDIF
	  ___NAZION    = FOR_NAZION
	  M.PAR_VALUTA = FOR_VALUTA
	ENDIF
...
*!*	WAIT WINDOWS "SCADENZA : CONTO "+ __CONTO__ + " NUMFAT " + __NUMDOC__ + " IMPORTO " + _IMPORTO NOWAIT
*!*	IF __CONTO__  = '0701410100011' AND ALLTRIM(__NUMDOC__) = '1/2' AND _IMPORTO = '750.20'
*!*	SET STEP ON 
*!*	ENDIF 
