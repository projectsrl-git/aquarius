...
cSql="select * from U_FTP_AN where ftp_tipcom='Z' and ftp_execute=1 and ftp_codcas = '"+ALLTRIM(_codhw)+"' order by ftp_codpdv"
...
do while !eof()
	...
	*!* --------------------------------------------------
	IF !w_creadir("\Log\LogFtp")
		RETURN
	ELSE
		_file_log	=	"\Log\LogFtp\Log_PDV" + _codPDV + "Image_"+ALLTRIM(_codhw)+"." + DTOS(DATE())+".log"
		IF !FILE(_file_log)
		...
		* ------------------------------------------------------------
		...
		WITH objFtp 
		    .SetCurrentDir(_dirDa) 
				...
				*******************************************************
				IF !EMPTY(_nomeFile)
					_nfile=OCCURS(";",_nomeFile)+1
					FOR _jx=1 TO _nfile
						_getfile = STR_EXTRACT(_nomeFile,";",_jx)
						IF !EMPTY(_getfile)
							WAIT WINDOW "Inizio ricezione di : "+ALLTRIM(_getfile)+ " dal PDV: " + _desPDV nowait
						    FOR j=1 TO 10
 							    IF .GetFromFtp(_getfile, _dirA+"TMP"+_getfile)
		  				    		.DeleteFile(_getfile)
		  				    		_cmd="copy file "+_dirA+"TMP"+_getfile+" to "+_dirA+_getfile
		  				    		&_cmd
		  				    		_cmd="copy file "+_dirA+"TMP"+_getfile+" to \AllImage\"+_codPDV+_getfile
		  				    		&_cmd
 							    	EXIT
 							    ENDIF
 							NEXT 
						    
							WAIT WINDOW "FINE ricezione di : "+ALLTRIM(_getfile)+ " dal PDV: " + _desPDV  nowait				    	
						ENDIF 
					NEXT 
				...
				IF !EMPTY(_nomeFile)
					**WAIT WINDOW "Sto inviando: "+_nomeFile nowait
			    	IF .PutToFtp(_dirDa+ALLTRIM(_nomeFile), ALLTRIM(_nomeFile)) 
			    		...
			    		** Creazione di un file di backup avente per nome per es. : "\Backup\Sent\esempio.txt.20030716_163459.005"
			    		_fileBKUP 	 = "\Backup\Sent\" + _codPDV + "_" + DTOS(DATE()) + "." + STRTRAN(TIME(),":","") + "." + ALLTRIM(_nomeFile)  

			    		** Creazione cartella di bakup file inviati
						IF !w_creadir("\Backup\Sent")
							RETURN
						ENDIF
						
			    		** Copia dei file inviati nella cartella di backup
			    		COPY file &_fileinviato to &_fileBKUP  
				...
				ELSE
				    ...
				    scan
						**WAIT WINDOW "Sto inviando: "+ALLTRIM(cur_locale.filename) nowait
			    		IF .PutToFtp(_dirDa+ALLTRIM(cur_locale.filename), ALLTRIM(cur_locale.filename)) 
				    		_fileinviato = _dirDa+ALLTRIM(cur_locale.filename)
				    		
				    		** Creazione di un file di backup avente per nome per es. :  "esempio.txt.20030716_163459.005"
				    		_fileBKUP 	 = "\Backup\Sent\" + _codPDV + "_" + DTOS(DATE()) + "." + STRTRAN(TIME(),":","") + "." + ALLTRIM(cur_locale.filename) 
...
* return .F. to cancel file transfer 
	_hd=FOPEN("\Log\LogFtp\Log_PDV" + _codPDV + "Image_"+ALLTRIM(_codhw)+"." + DTOS(DATE())+".log",2)
	=FSEEK(_hd,0,2)
