...
DO WHILE .T.
   ...
   **--> 
   ...
   DO WHILE .T.
      ...
      SCATTER MEMVAR
      DAVX=DAVD  
      DO MOVCRINC  WITH DAVX,M.RIC_TOTALE,0,'NO',M.RIC_CONTO,TOP_TOP,M.DOC,M.DTD,.T.,M.OLFAT,XVALUTA,XCAMBIO,M.RIC_NPART
      SELECT CU_MAN_DD
      SKIP+1
   ENDDO 
...
******************************************************************************
PARA DA_AV,IMP,TAS,IV,CON,TOP,NDOC,DTD,PARTITARIO,NPRO,YVALUTA,YCAMBIO,YNPART
...
ENDIF
IF SUBSTR(M.RIC_DATSCA,3,1) = '/'
   REPLACE PAR_DTSCAD  WITH RIBALTA2(M.RIC_DATSCA)
   REPLACE PAR_DATPAG  WITH RIBALTA2(M.RIC_DATSCA)
ELSE
...
REPLACE PAR_NPART   WITH YNPART   &&VAL(NDOC)
...
REPLACE PAR_COSATRATTA 		WITH M.RIC_COSATRATTA
REPLACE PAR_GRADOIMPORTANZA WITH M.RIC_GRADOIMPORTANZA
REPLACE PAR_FORMAPAGAMENTO 	WITH M.RIC_FORMAPAGAMENTO
REPLACE PAR_TIPOCONTRATTO 	WITH M.RIC_TIPOCONTRATTO
REPLACE PAR_CODACQ			WITH M.RIC_CODACQ
REPLACE PAR_PRIPAGA			WITH M.RIC_PRIPAGA
* --------  Classifica la partita -------- *
...
=CREA_SALDI_CF("F",CON,ALLTRIM(STR(YNPART,8,0)),"XXXXXXXXXX","CREA")
