...
IF PUB_LIQDTDOC
	...
	*** DEFINISCE I CURSORI DI SUPPORTO USATI PER LA GESTIONE DELLA LIQUIDAZIONE IVA PER DATA DOCUMENTO
	CREATE CURSOR cur_regis_DTDOC ;
		(____NUMPRO C(13))
	INDEX ON ____NUMPRO				TAG NUMPRO___

	IF !ExecCommand("select * from u_iva_cl_DTDOC where 0=1","cu_iva_cl_DTDOC")			&& bollato iva clienti
	   return(.f.)
	ENDIF
	IF !ExecCommand("select * from u_ivf_cl_DTDOC where 0=1","cu_ivf_cl_DTDOC")			&& bollato iva fornitori
	   return(.f.)
	ENDIF
ENDIF
...
**=> ERASMO - INIZIO 09/07/2019
IF PUB_LIQDTDOC
	DO IVAVENDI_DTDOC	&&  CARICAMENTO BOLLATO IVA VENDITE PER LIQUIDAZIONE IN COMPETENZA DETRAIBILITA' PER DATA DOCUMENTO
	DO IVAACQUI_DTDOC	&&  CARICAMENTO BOLLATO IVA ACQUISTI PER LIQUIDAZIONE IN COMPETENZA DETRAIBILITA' PER DATA DOCUMENTO
ENDIF
**=> ERASMO - FINE 09/07/2019
...
**=> ERASMO - INIZIO 09/07/2019
IF PUB_LIQDTDOC
	DO RAGGRUPPA_FATTURE_DTDOC
	
	IF !ExecRW('','u_iva_cl_DTDOC','I','','CU_IVA_CL_DTDOC')
	   return(.f.)
	ENDIF
	IF !ExecRW('','u_ivf_cl_DTDOC','I','','CU_IVF_CL_DTDOC')
	   return(.f.)
	ENDIF
ENDIF
**=> ERASMO - FINE 09/07/2019
...
*!*	.......................................................................................... VERIFICA CLIENTI/FORNITORI
   ...
   IF PUB_CONTOSIMILCEE 
   	   IF VERYCF_CERCA_CONTO_SU_CONTI(MOV_CONTO) = "C" OR VERYCF_CERCA_CONTO_SU_CONTI(MOV_CONTO) = "F" 
   	   ELSE
	      SKIP +1
	      LOOP
	   ENDIF   	   
   ...
   IF PUB_CONTOSIMILCEE 
   	   IF VERYCF_CERCA_CONTO_SU_CONTI(MOV_CONTO) = "C" 
...
*!*			 ENDIF
		...
		**=> ERASMO/FABIANO - FINE 29/12/2017
		 ...
		 * SE MOV_IMP <> 0 E MOV_IMPOST = 0 CASO DI IVA IN ESENZIONE
		 *IF CAMBIO(MOV_VALUTA,MOV_IMP,MOV_CAMBIO) <> 0 
		 IF CAMBIO(MOV_VALUTA,MOV_IMP,MOV_CAMBIO) <> 0 AND VAL(ALIQ) = 0  && ERASMO - 14/06/2019
		 	IF CAMBIO(MOV_VALUTA,MOV_IMPOST,MOV_CAMBIO) = 0
	           IMPONIBILE  = CAMBIO(MOV_VALUTA,MOV_IMP,MOV_CAMBIO)
		       IMPOSTA     = CAMBIO(MOV_VALUTA,MOV_IMPOST,MOV_CAMBIO)
...
*!*	  PER FATTURE CON ACCONTI SCARTA I MOVIMENTI RELATIVI AI PAGAMENTI AGGANCIATI
   ...
   ENDIF
    ...
    IF PUB_CONTOSIMILCEE 
   	   IF VERYCF_CERCA_CONTO_SU_CONTI(MOV_CONTO) = "F" 
   	   ELSE
	      SKIP +1
	      LOOP
	   ENDIF   	   
   ...
   *   più volte l'imposta, si deve verificare che la registrazione in esame NON sia già stata elaborata.
   ...
   IF !EOF()
	  ...
	  DO WHILE .T.
		...
		**=> ERASMO - INIZIO 22/07/2020
		*** CAMBIO SEGNO IMPORTI PER CASI SPECIFICI
		SELECT MOV_CONT
		IF !MOV_ANT_CAMB_SEGNO
			...
			SELECT MOV_CONT
			IF NOTAC <> 'S' AND MOV_TMOV = 'A'
				REPLACE  MOV_IMP    WITH MOV_IMP    * -1
				REPLACE  MOV_IMPOST WITH MOV_IMPOST * -1
			ENDIF
			IF NOTAC = 'S' AND MOV_TMOV = 'D'
				REPLACE  MOV_IMP    WITH MOV_IMP    * -1
				REPLACE  MOV_IMPOST WITH MOV_IMPOST * -1
			ENDIF
		ENDIF
		**=> ERASMO- FINE 22/07/2020
...
*!*			 ENDIF
         ...
         ENDIF
  	     ...
  	     **=> ERASMO - INIZIO 10/03/2020 - GESTIONE ANTICIPI CON CAMBIO SEGNO
  	     SELECT MOV_CONT
	     IF MOV_ANT_CAMB_SEGNO
	          REPLACE  MOV_IMP    WITH MOV_IMP    * -1
	          REPLACE  MOV_IMPOST WITH MOV_IMPOST * -1
...
DO WHILE .NOT. EOF()
	...
	*** 2015/06/16 SERGIO/ERASMO FORZATURA PER GESTIRE PROBLEMATICHE DI CONVERSIONE
	...
	IF seek_std('para',v_seek,v_chiave,'CUR_PARA_IVA')
		 ...
		 **=> ERASMO - INIZIO 21/12/2020
		 *** SOLO I CODICI IVA PREDISPOSTI PER L'IVA IN SOSPENSIONE
         IF UPPER(ALLTRIM(DESIVA)) == "SOSP"
         ELSE
	     	SELECT PART_CLI_INCASSATE
	    	SKIP +1
	    	LOOP        
		 ENDIF
		 **=> ERASMO - FINE 21/12/2020
	ENDIF
...
IMPONIBILEX = CAMBIO(MOV_VALUTA,NW_IMPONI,MOV_CAMBIO,"",.F.,20)  && ERASMO - 16/11/2020
...
_PERCENTUALE_DEDUCIBILE = 0  && ERASMO - 16/11/2020
...
ELSE
	*** NUOVA GESTIONE CODICE IVA ABBINATO
	...
	seek_std('para',v_seek,v_chiave,'CUR_PARA')
   	...
   	_PERCENTUALE_DEDUCIBILE = VAL(SUBSTR(LIBERA,110,3))  && ERASMO - 16/11/2020
	v_seek   = 'IVA'+ALLTRIM(COD_IVA ) 
	IF seek_std('para',v_seek,v_chiave,'CUR_PARA')
  		SELECT CUR_PARA
   		DES_IVA   = SUBSTR(DESCRI,1,30)
...
**=> ERASMO - INIZIO 16/11/2020
*** SE LA PERCENTUALE DEDUCIBILE E' 50%, TUTTI I VALORI DI INDEDUCIBILITA' DEVONO NECESSARIAMENTE RISULTARE SPECULARI, QUINDI UGUALI A QUELLI DI DEDUCIBILITA'
IF _PERCENTUALE_DEDUCIBILE = 50
ELSE
	*IMPON_DED    = IMPONIBILE
	IMPON_DED    = IMPONIBILEX
	IMPOS_DED    = IMPOSTA
	TOT_LORD     = _TOT_DOC - IMPON_DED - IMPOS_DED
	IMPONIBILE   = (TOT_LORD * 100) / (PERIVA + 100)
...
x_mese = "iva_mese = '" + MESEDAL + "'"
x_cond = x_soci + " and " + x_anno + " and " + x_mese
CSQL   = "delete from U_IVA_CL_DTDOC where " + x_cond
IF !ExecCommand(CSQL,"U_IVA_CL_DTDOC")
   RETURN (.f.)
ENDIF 
...
x_mese = "ito_mese = '" + MESEDAL + "'"
x_cond = x_soci + " and " + x_anno + " and " + x_mese 
CSQL   = "delete from U_IVA_TO_DTDOC where " + x_cond
IF !ExecCommand(CSQL,"U_IVA_TO_DTDOC")
   RETURN (.f.)
ENDIF 
...
*!*			 ENDIF
		...
		**=> ERASMO/FABIANO - FINE 29/12/2017
		 ...
		 * SE MOV_IMP <> 0 E MOV_IMPOST = 0 CASO DI IVA IN ESENZIONE
		 IF CAMBIO(MOV_VALUTA,MOV_IMP,MOV_CAMBIO) <> 0 
		 	IF CAMBIO(MOV_VALUTA,MOV_IMPOST,MOV_CAMBIO) = 0
		       ...
		       IMPOSTA     = CAMBIO(MOV_VALUTA,MOV_IMPOST,MOV_CAMBIO)
		       TOTALE      = CAMBIO(MOV_VALUTA,MOV_IMP,MOV_CAMBIO) + CAMBIO(MOV_VALUTA,MOV_IMPOST,MOV_CAMBIO)
			   DO IVACLIES_DTDOC	&&  GESTIONE IVA ESENTE PER CLIENTI
			   SELECT MOV_CONT
			   SKIP +1
			   LOOP
			ENDIF
     	 ...
     	 ** === > 19/01/2004 NUOVA GESTIONE MATERIALE PUBBLICITARIO------FINE
		    IF FAT = 'C'  
	 		   DO CORRISP_DTDOC     && GESTIONE CORRISPETTIVI
               XXX_FATNOT = 'C'
            ELSE
		       IF NOTAC = 'S'  
		          T_IPO = 'N'
               ...
               x_tipo = "iva_top = '" + ALLTRIM(WTOP) + "'"  && ERASMO - 18/12/2017
               x_cond = x_soci + " and " + x_anno + " and " + x_mese + " and " + x_chia + " and " + x_codi + " and " + x_fatt + " and " + x_biva + " and " + x_clie + " and " + x_tipo
               CSQL   = "select * from u_iva_cl_DTDOC where " + x_cond + " order by iva_codsoc,iva_anno,iva_mese,iva_chiave,iva_codiva,iva_fatnot" 
               IF !ExecRW(CSQL,"u_iva_cl_DTDOC","R")
                  RETURN (.f.)
               ENDIF 
  	 	       SELECT U_IVA_CL_DTDOC
  	 	       GO TOP
  	 	       IF EOF()
 			      APPEND BLANK
 			      REPLACE    MOV_NUMPRO    WITH __MOV_NUMPRO__  && ERASMO/SERGIO - 01/08/2017
               ...
               ** CREA MOVIMENTO IMPONIBILE ESENTE NEL CASO DI ARTICOLI PUBB.CHE SONO CONFLUITI NELLE VENDITE
     	       IF OK_ESE		
				  ...
				  REPLACE    IVA_BOLLATOIVA WITH WBOLLATOIVA
	           ENDIF		
               IF !ExecRW(cSql,"U_IVA_CL_DTDOC","W")
                  return(.f.)
               ENDIF
            ENDIF  
         ELSE
         ...
         *IF CEE = "E" OR CEE = 'R' 
         ...
         x_fatt = "ito_fatnot = '" + ALLTRIM(XXX_FATNOT) + "'"
         x_cond = x_soci + " and " + x_anno + " and " + x_mese + " and " + x_codi  + " and " + x_tipo + " and " + x_fatt
         CSQL   = "select * from u_iva_to_DTDOC where " + x_cond + " order by ito_codsoc,ito_anno,ito_mese,ito_codiva,ito_clifor,ito_fatnot" 
         IF !ExecRW(CSQL,"u_iva_to_DTDOC","R")
            RETURN (.f.)
         ENDIF 
         SELECT U_IVA_TO_DTDOC
         GO TOP
         IF !EOF()
            DO IVA_REP  
         ELSE
            ...
            DO IVA_INS
         ENDIF 
         IF !ExecRW(cSql,"U_IVA_TO_DTDOC","W")
            return(.f.)
         ENDIF
...
x_mese = "ivf_mese = '" + MESEDAL + "'"
x_cond = x_soci + " and " + x_anno + " and " + x_mese
CSQL   = "delete from U_IVF_CL_DTDOC where " + x_cond
IF !ExecCommand(CSQL,"U_IVF_CL_DTDOC")
   RETURN (.f.)
ENDIF 
		...
		ENDIF 	
		 ...
		 IF TIPOIVA = "I"     && TRATTA IVA INDEDUCIBILE AL N %
			IF PERIVAI <> 100
		       ...
		       TOTALE      = CAMBIO(MOV_VALUTA,MOV_IMP+MOV_IMPOST,MOV_CAMBIO)
		       WFATT       = CAMBIO(MOV_VALUTA,MOV_IMP+MOV_IMPOST,MOV_CAMBIO)
  		       DO IVAFORES_DTDOC	&&  GESTIONE IVA IMPONIBILE AL N% 
			   SELECT MOV_CONT
			   SKIP +1
			   LOOP
			ENDIF
...
************************
PROCEDURE IVACLIES_DTDOC	&&  GESTIONE IVA ESENTE PER CLIENTI
************************
...
************************
PROCEDURE IVAFORES_DTDOC	&&  GESTIONE IVA INDEDUCIBILE A N% PER FORNITORI
************************
...
************************
PROCEDURE CORRISP_DTDOC       && GESTIONE MOVIMENTI IVA CORRISPETTIVI
************************
...
***********************************
PROCEDURE RAGGRUPPA_FATTURE_DTDOC
***********************************
** RAGRUPPAMENTO CURSORI PER DOCUMENTO/IVA
*!*	               x_soci = "iva_codsoc = '" + PUB_CODSOC + "'"
*!*	               x_anno = "iva_anno = '" + PUB_ANNO + "'"
...
IF !ExecCommand("select * from u_ivf_cl_DTDOC where 0=1","R_ivf_cl_DTDOC")			&& bollato iva fornitori
   return(.f.)
ENDIF

SELECT R_IVF_CL_DTDOC
*INDEX ON IVF_CODSOC + IVF_ANNO + IVF_MESE + ALLTRIM(STR(IVF_CHIAVE)) + ALLTRIM(IVF_CODIVA) + IVF_FATNOT  TAG RAGGRUPPA
INDEX ON IVF_CODSOC + IVF_ANNO + IVF_MESE + ALLTRIM(STR(IVF_CHIAVE)) + ALLTRIM(IVF_CODIVA) + IVF_FATNOT + ALLTRIM(IVF_NUMFAT)  TAG RAGGRUPPA  && ERASMO - 18/12/2017
